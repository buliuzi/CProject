...
int CreatList2(CITY_NODE **phead) {
    ...
    if ((pFile = fopen(gp_city_info_filename, "rb")) == NULL) {
        printf("城市信息链表数据文件打开失败!\n");
        return re;
    }
    printf("城市信息链表数据文件打开成功!\n");
...
void ShowMenu() {
    ...
    for (i = 0; i < 6; i++) /*在窗口第一行第一列处输出主菜单项*/
    {
...
int PopInputMenu(char *plabel_name[], int label_num, char *ppcondition[], int hot_area_num) {
    ...
    aLoc[n - 1].X = rcPop.Left + (pos.X - strlen(plabel_name[n - 1])) / 2+15;
    ...
    for (i = 0; i < hot_area_num - 2; i++) {
        aArea[i].Left = aLoc[i + 1].X + strlen(plabel_name[i + 1])+2;
        aArea[i].Top = aLoc[i + 1].Y;
        aArea[i].Right = aLoc[i + 1].X + 48;
...
int PopCityInputMenu(char *plabel_name[], int label_num, char *ppcondition[], int hot_area_num, int inputLen[]) {
    LABEL_BUNDLE labels;
    HOT_AREA areas;
    SMALL_RECT rcPop;
    COORD pos;
    ...
    for (i = 0; i < hot_area_num - 2; i++) {
        ...
        aArea[i].Top = aLoc[i + 1].Y;
        aArea[i].Right = aLoc[i + 1].X + inputLen[i];
        aArea[i].Bottom = aLoc[i + 1].Y;
    }
    aArea[hot_area_num - 2].Left = aLoc[hot_area_num - 1].X;
    aArea[hot_area_num - 2].Top = aLoc[hot_area_num - 1].Y;
...
int DealInput2(HOT_AREA *pHotArea, int *piHot, char **ppcondition) {
    ...
    for (i = 0; i < TxtHotAreaNum; i++) {
        ppcondition[i] = (char *) malloc(60 * sizeof(char));
        ppcondition[i][0] = '\0';
    }
...
BOOL ShowResult(char **pString, int n,int col )
{
    ...
    for (i=1; i<col; i++)
    {
        aLoc[i+1].X = aLoc[i].X + strlen(pString[i]);/*使相邻字符之间相隔固定字符数*/
        aLoc[i+1].Y = aLoc[i].Y;
    }
    for (j=1; j<row; j++)
    {
        for (i=0; i<col; i++)
        {
            ...
            aLoc[j*col+i+1].Y = rcPop.Top + 3 + j;
        }
    }
...
BOOL ShowResult2(char **pString, int n,int col )
{
    ...
    for (i=1; i<col; i++)
    {
        aLoc[i+1].X = aLoc[i].X + strlen(pString[i])+5;/*使相邻字符之间相隔固定字符数*/
        aLoc[i+1].Y = aLoc[i].Y;
    }
    for (j=1; j<row; j++)
    {
...
int ShowSearchResult2(char **plabel_name, int num, int choice_num) {
    LABEL_BUNDLE labels;
    HOT_AREA areas;
    SMALL_RECT rcPop;
    COORD pos;
    ...
    aLoc[0].X = (pos.X - strlen(plabel_name[0])) / 2 + rcPop.Left;
    aLoc[0].Y = rcPop.Top + 1;
    /******设置标签束的输出位置*****/
    for (i = 1; i < n; i++) {
        aLoc[i].X = rcPop.Left + 12;
        aLoc[i].Y = rcPop.Top + 2 + i;
    ...
    areas.num = n-1;
    SMALL_RECT aArea[n-1];
    char aSort[n-1];
    char aTag[n-1];
    for (i = 0; i < choice_num; i++) {
        aArea[i].Left = aLoc[i+1].X;
        aArea[i].Top = aLoc[i+1].Y;
        aArea[i].Right = aLoc[i+1].X + strlen(plabel_name[i+1]) - 1;
        aArea[i].Bottom = aLoc[i+1].Y;
        aSort[i] = 0;
        aTag[i] = i + 1;
    }
...
BOOL ExeFunction(int m, int s) {
    ...
    BOOL(*pFunction[ga_sub_menu_count[0] + ga_sub_menu_count[1] + ga_sub_menu_count[2] + ga_sub_menu_count[3] +
                    ga_sub_menu_count[4] + ga_sub_menu_count[5]])(void);
    int i, loc;
    ...
    pFunction[10] = StatusCityInfo;

    pFunction[11] = RandomCityView;
    pFunction[12] = RandomRegionView;
    pFunction[13] = RandomSpotView;

    pFunction[14] = HelpTopic;
    pFunction[15] = NULL;
    pFunction[16] = AboutThis;
...
BOOL InsertCityNodeSubMenu(void) {
    ...
    if (iHot == -1) {
        ...
        if (iHot == 5) {
            PopOff();
            if (strlen(ppcondition[0]) == 0 || strlen(ppcondition[1]) == 0 || strlen(ppcondition[2]) == 0
                                                  || strlen(ppcondition[3]) == 0) {
                char *error_plabel_name[] = {"不可以有空白输入哦",
                                             "确认"
                };
        ...
        if (iHot == 1) {
            ...
            if (aHot == 2) {
                ...
                if (pCityNode == NULL) {
                    ...
                    pCityNode = SeekCityNodeByID(gp_head2, pCityNode->city_id);
                    char table[100];
                    table[0] = '\0';
        ...
        if (iHot == 1) {
            ...
            if (aHot == 2) {
                ...
                if (pCityNode == NULL) {
                    ...
                    int inputNum = 5;
                    char *ppcondition[inputNum];
                    int choice = ShowSearchResult2(plabel_name,5,4);
                    if(choice==1){
                        PopOff();
                        char table2[100];
                        table2[0] = '\0';
                        strcat(table2, "原城市名称为：");
                        strcat(table2, pCityNode->name);
                        char *plabel_name2[] = {"",
                                               "新的城市名称",
                                               "确定    取消"
                        };
                        plabel_name2[0] = table2;
                        int n = 3;
                        int inputNum = 1;
                        char *ppcondition[inputNum];
                        int iHot = PopInputMenu(plabel_name2, n, ppcondition, inputNum + 2);
                        if (iHot == 2) {
                            if (strlen(ppcondition[0])==0) {
                                PopOff();
                                char *error_plabel_name[] = {"修改信息不能为空！",
                                                             "确认"
                                };
                                ShowModule(error_plabel_name, 2);
                                return bRet;
                            }
                            strcpy(pCityNode->name, ppcondition[0]);
                            PopOff();
                            char *error_plabel_name[] = {"修改成功！",
                                                         "确认"
                            };
                        ...
                        char table2[100];
                        table2[0] = '\0';
                        strcat(table2, "原城市监督电话为：");
                        strcat(table2, pCityNode->jiandu_num);
                        char *plabel_name[] = {"",
                                               "新的城市监督电话",
                                               "确定    取消"
                        };
                        plabel_name[0] = table2;
                        int n = 3;
                        int inputNum = 1;
                        char *ppcondition[inputNum];
                        int iHot = PopInputMenu(plabel_name, n, ppcondition, inputNum + 2);
                        if (iHot == 2) {
                            ...
                            strcpy(pCityNode->jiandu_num, ppcondition[0]);
                            PopOff();
                            char *error_plabel_name[] = {"修改成功！",
                                                         "确认"
                            };
                        ...
                        char table2[100];
                        table2[0] = '\0';
                        strcat(table2, "原城市咨询电话为：");
                        strcat(table2, pCityNode->zixun_num);
                        char *plabel_name[] = {"",
                                               "新的城市咨询电话",
                                               "确定    取消"
                        };
                        ...
                        if (iHot == 2) {
                            ...
                            strcpy(pCityNode->zixun_num, ppcondition[0]);
                            PopOff();
                            char *error_plabel_name[] = {"修改成功！",
                                                         "确认"
                            };
...
BOOL InsertScenicAreaNodeSubMenu(void) {
    ...
    if (iHot == -1) {
        ...
        if (iHot == 8) {
            PopOff();
            if (strlen(ppcondition[0]) == 0 || strlen(ppcondition[1]) == 0 || strlen(ppcondition[2]) == 0
                || strlen(ppcondition[3]) == 0|| strlen(ppcondition[4]) == 0 || strlen(ppcondition[5]) == 0
                   || strlen(ppcondition[6]) == 0) {
                char *error_plabel_name[] = {"不可以有空白输入哦",
                                             "确认"
                };
                ShowModule(error_plabel_name, 2);
        ...
        if (iHot == 1) {
            ...
            if (aHot == 2) {
                ...
                if (pRegionNode == NULL) {
                    ...
                    pRegionNode = SeekRegionNodeByID(gp_head2,pRegionNode->region_id);
                    char table[100];
                    table[0] = '\0';
                    strcat(table, "正在修改名称为");
                    ...
                    int inputNum = 6;
                    char *ppcondition[inputNum];
                    int choice = ShowSearchResult2(plabel_name,n,inputNum);
                    if(choice==1){
                        PopOff();
                        char table2[100];
                        table2[0] = '\0';
                        strcat(table2, "原景区名称为：");
                        strcat(table2, pRegionNode->name);
                        char *plabel_name2[] = {"",
                                                "新的景区名称",
                                                "确定    取消"
                        };
                        ...
                        char table2[100];
                        table2[0] = '\0';
                        strcat(table2, "原景区级别为：");
                        strcat(table2, pRegionNode->level);
                        char *plabel_name2[] = {"",
                                                "新的景区级别",
                                                "确定    取消"
                        };
                        ...
                        if (iHot == 2) {
                            ...
                            strcpy(pRegionNode->level, ppcondition[0]);
                            PopOff();
                            char *error_plabel_name[] = {"修改成功！",
                                                         "确认"
                            };
                    ...
                    }else if(choice==3)
                    {
                        ...
                        char table2[100];
                        table2[0] = '\0';
                        strcat(table2, "原景区地址为：");
                        strcat(table2, pRegionNode->address);
                        char *plabel_name2[] = {"",
                                                "新的景区地址",
                                                "确定    取消"
                        };
                        ...
                        if (iHot == 2) {
                            ...
                            strcpy(pRegionNode->address, ppcondition[0]);
                            PopOff();
                            char *error_plabel_name[] = {"修改成功！",
                                                         "确认"
                            };
                        ...
                        char table2[100];
                        table2[0] = '\0';
                        strcat(table2, "原门票价格为：");
                        strcat(table2, pRegionNode->price);
                        char *plabel_name2[] = {"",
                                                "新的门票价格",
                                                "确定    取消"
                        };
                        ...
                        if (iHot == 2) {
                            ...
                            strcpy(pRegionNode->price, ppcondition[0]);
                            PopOff();
                            char *error_plabel_name[] = {"修改成功！",
                                                         "确认"
                            };
                        ...
                        char table2[100];
                        table2[0] = '\0';
                        strcat(table2, "原开放时间为：");
                        strcat(table2, pRegionNode->opentime);
                        char *plabel_name2[] = {"",
                                                "新的开放时间",
                                                "确定    取消"
                        };
                        ...
                        if (iHot == 2) {
                            ...
                            strcpy(pRegionNode->opentime, ppcondition[0]);
                            PopOff();
                            char *error_plabel_name[] = {"修改成功！",
                                                         "确认"
                            };
            ...
            if (aHot == 2) {
                ...
                if (pRegionNode == NULL) {
                    ...
                    table[0] = '\0';
                    strcat(table, "正在修改ID为");
                    strcat(table, pRegionNode->region_id);
                    strcat(table, "的景区信息：");
                    char *plabel_name[] = {
                            "",
                            "修改景区名称",
        ...
        if (iHot == 7) {
            ...
            if (strlen(ppcondition[0]) == 0 || strlen(ppcondition[1]) == 0 || strlen(ppcondition[2]) == 0
                ...
                if (spotFound != NULL) {
                    ...
                    strcpy(pSpotNode->zixun_num, ppcondition[3]);
        ...
        if (iHot == 1) {
            ...
            if (aHot == 2) {
                ...
                if (pSpotNode == NULL) {
                    ...
                    pSpotNode = SeekSpotNodeByID(gp_head2, pSpotNode->spot_id);
                    char table[100];
                    table[0] = '\0';
                    strcat(table, "正在修改名称为");
                    ...
                    if(choice==1){
                        ...
                        char table2[100];
                        table2[0] = '\0';
                        strcat(table2, "原景点名称为：");
                        strcat(table2, pSpotNode->name);
                        char *plabel_name2[] = {"",
                                                "新的景点名称",
                                                "确定    取消"
                        };
                        ...
                        char table2[100];
                        table2[0] = '\0';
                        strcat(table2, "原咨询电话为：");
                        strcat(table2, pSpotNode->zixun_num);
                        char *plabel_name2[] = {"",
                                                "新的咨询电话",
                                                "确定    取消"
                        };
                        ...
                        if (iHot == 2) {
                            ...
                            strcpy(pSpotNode->zixun_num, ppcondition[0]);
                            PopOff();
                            char *error_plabel_name[] = {"修改成功！",
                                                         "确认"
                            };
                        ...
                        char table2[100];
                        table2[0] = '\0';
                        strcat(table2, "原浏览时间为：");
                        strcat(table2, pSpotNode->opentime);
                        char *plabel_name2[] = {"",
                                                "新的浏览时间",
                                                "确定    取消"
                        };
                        ...
                        if (iHot == 2) {
                            ...
                            strcpy(pSpotNode->opentime, ppcondition[0]);
                            PopOff();
                            char *error_plabel_name[] = {"修改成功！",
                                                         "确认"
                            };
        ...
        if (iHot == 1) {
            ...
            if (aHot == 2) {
                ...
                if (pRegionNode == NULL) {
                    ...
                    CITY_NODE *p = SeekCityNodeByID(gp_head2, pRegionNode->city_id);
            ...
            if (aHot == 2) {
                ...
                if (pRegionNode == NULL) {
                    ...
                    plabel_ret2[14] = pRegionNode->opentime;
                    plabel_ret2[15] = "确定";
                    ShowResult(plabel_ret2, 7 * 2 + 2, 2);
                }
    ...
    if (iHot == -1) {
        ...
        if (iHot == 1) {
            ...
            if(pSpotNode==NULL){
                ...
                plabel_ret2[7] = "咨询电话:";
                plabel_ret2[8] = pSpotNode->zixun_num;
                plabel_ret2[9] = "浏览时间:";
...
BOOL StatusCityInfo() {
    ...
    int region5a_num[cityNum], region4a_num[cityNum];
    int region_num[cityNum], spot_num[cityNum];
    memset(region5a_num, 0, cityNum * sizeof(int));
    memset(region4a_num, 0, cityNum * sizeof(int));
    memset(region_num, 0, cityNum * sizeof(int));
    memset(spot_num, 0, cityNum * sizeof(int));
    char city_name_shuzu[cityNum][100] ;
    char region_num_shuzu[cityNum][100];
    char region5a_num_shuzu[cityNum][100];
    char region4a_num_shuzu[cityNum][100];
    char spot_num_shuzu[cityNum][100];

    int i = 0;
    pCityNode = gp_head2;
    while(pCityNode!=NULL){
        strcpy(city_name_shuzu[i], pCityNode->name);
        pRegionNode = pCityNode->rnext;
        while(pRegionNode!=NULL){
            regionNum++;
            region_num[i]++;
            if (strcmp(pRegionNode->level, "AAAAA"))
                region5a_num[i]++;
            else if(strcmp(pRegionNode->level, "AAAA"))
                region4a_num[i]++;
            else{}
        ...
        itoa(region_num[i],region_num_shuzu[i],10);
        itoa(region5a_num[i],region5a_num_shuzu[i],10);
        itoa(region4a_num[i], region4a_num_shuzu[i], 10);
        itoa(spot_num[i],spot_num_shuzu[i],10);
        pCityNode = pCityNode->next;
        i++;
    }

    i=0;
    char *plabel_ret2[5 * (cityNum + 1) + 2];
    plabel_ret2[i++] = "信息统计结果";
    plabel_ret2[i++] = "城市名称";
    plabel_ret2[i++] = "景区个数";
    plabel_ret2[i++] = "5A景区个数";
    plabel_ret2[i++] = "4A景区个数";
    plabel_ret2[i++] = "景点个数";

    int j = 0;
    while(j < cityNum)
    {
        plabel_ret2[i++] = city_name_shuzu[j];
        plabel_ret2[i++] = region_num_shuzu[j];
        plabel_ret2[i++] = region5a_num_shuzu[j];
        plabel_ret2[i++] = region4a_num_shuzu[j];
        plabel_ret2[i++] = spot_num_shuzu[j];
        j++;
    }
    plabel_ret2[i++] = "确定";
    ShowResult2(plabel_ret2, i, 5);
    return bRet;
}
        ...
        if (iHot == 1) {
            ...
            if (aHot == 2) {
                ...
                if (pSpotNode == NULL) {
                    ...
                    char *plabel_ret2[2 * 6 + 2];
                    REGION_NODE *p = SeekRegionNodeByID(gp_head2, pSpotNode->region_id);
                    plabel_ret2[0] = "你查询的景点信息";
                    plabel_ret2[1] = "景点名称:";
                    plabel_ret2[2] = pSpotNode->name;
                    plabel_ret2[3] = "景  点ID:";
        ...
        for (pRegionNode = pCityNode->rnext; pRegionNode != NULL; pRegionNode = pRegionNode->next) {
            ...
            while (pSpotNode != NULL) {
                if (strcmp(pSpotNode->name, name) == 0) {
                    ...
                    strcpy(tmp->zixun_num, pSpotNode->zixun_num);
